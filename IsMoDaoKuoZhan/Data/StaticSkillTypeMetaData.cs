    ...
    internal class StaticSkillTypeMetaData
    {
        ...
        public void Load()
        {
            ...
            foreach (var skillType in _staticSkillTypes)
            {
                ...
                else
                {
                    ...
                    IsMoDaoKuoZhanMain.Warning($"功法类型 [{skillType.Id}] 已存在，请检查 Mod 列表");
                }
            }
        ...
        private void LoadFromJson()
        {
            try
            {
                ...
                {
                    IsMoDaoKuoZhanMain.Error($"未找到 StaticSkillType.json 文件: {jsonPath}");
                    return;
                }
            ...
            {
                IsMoDaoKuoZhanMain.Error($"加载 StaticSkillType.json 失败: {ex.Message}");
            }
        }
        ...
        public void AddStaticSkillType(ModStaticSkillType skillType)
        {
            ...
            {
                IsMoDaoKuoZhanMain.Warning("已存在问题");
                IsMoDaoKuoZhanMain.Warning($"功法类型 [{skillType.Id}] 已存在，将被覆盖");
