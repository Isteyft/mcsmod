    ...
    internal class AttackTypeMetaData
    {
        ...
        public void Load()
        {
            ...
            foreach (var attackType in _attackTypes)
            {
                ...
                {
                    IsMoXinXiangJuanMain.Warning("已存在冲突问题");
                    IsMoXinXiangJuanMain.Warning($"AttackType [{attackType.Id}] 发生冲突，请检查 Mod 列表");
                }
            }
        ...
        private void LoadFromJson()
        {
            try
            {
                ...
                {
                    IsMoXinXiangJuanMain.Error($"AttackType.json 文件未找到: {jsonPath}");
                    return;
                }
            ...
            {
                IsMoXinXiangJuanMain.Error($"加载 AttackType.json 失败: {ex.Message}");
            }
        }
        ...
        public void AddAttackType(ModAttackType attackType)
        {
            if (_attackTypes.Any(x => x.Id == attackType.Id))
            {
                ...
                IsMoXinXiangJuanMain.Warning($"AttackType [{attackType.Id}] 已存在，将被覆盖");
