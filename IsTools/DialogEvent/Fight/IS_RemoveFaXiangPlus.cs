...
{
    [DialogEvent("IS_RemoveFaXiangPlus")]
    public class IS_RemoveFaXiangPlus : IDialogEvent
    {
        public void Execute(DialogCommand command, DialogEnvironment env, Action callback)
        {
            int target = command.GetInt(0, 0); // 0表示玩家，1表示npc
            int id = command.GetInt(0, 1); // 0表示玩家，1表示npc
            ...
            for (int i = 0; i < avatarObj.transform.childCount; i++)
            {
                ...
                if (child.name.Contains($"Avater{id}_1(Clone)"))
                {
                    ...
                    IsToolsMain.LogInfo($"删除变身模型: {child.name}");
                    break;
                }
            }
