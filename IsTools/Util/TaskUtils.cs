...
namespace top.Isteyft.MCS.IsTools.Util
{
    ...
    public static class TaskUtil
    {
        ...
        public static bool TryAddTask(this int taskId)
        {
            ...
            {
                IsToolsMain.Log($"自定义任务添加失败，未查询到该任务数据: {taskId}");
                return false;
            }

            if (HasTask(taskId))
            {
                IsToolsMain.Log($"自定义任务添加失败，已拥有该任务: {taskId}");
                return false;
            }
        ...
        public static bool NextTask(this int taskId)
        {
            if (!HasTask(taskId))
            {
                IsToolsMain.Log($"任务阶段跳转失败，未拥有该任务: {taskId}");
                return false;
            }
            ...
            if (taskIndex == taskFinalIndex)
            {
                ...
                IsToolsMain.Log($"当前任务阶段为最终阶段，任务已完成: {taskId}");
            }
            else
            {
                // 当前任务并非最终阶段
                SetTaskIndex(taskId, taskIndex + 1);
                IsToolsMain.Log($"当前任务阶段已成功跳转: {taskId}");
            }
